                    ...
                    for image in images:
                        ...
                        if img is not None:
                            ...
                            if (count < (ipu.TOTAL_IMAGES * ipu.TRAIN_FACTOR * 0.01)):
                                print(
                                    'Train:--------- Label is {} and Count is {}'.format(label, count))
                                ...
                                print(
                                    'Test:--------- Label is {} and Count is {}'.format(label, count))
...
def mini_kmeans(k, descriptor_list):
    ...
    print('K means trained to get visual words.')
...
def get_histograms(discriptors_by_class, visual_words, cluster_model):
    ...
    print('Histograms succesfully created for %i classes.' %
          len(histograms_by_class))
...
def predict_svm(X_train, X_test, y_train, y_test):
    ...
    np.savetxt('submission_svm.csv', np.c_[range(1, len(y_test)+1), y_pred, y_test],
               delimiter=',', header='ImageId,PredictedLabel,TrueLabel', comments='', fmt='%d')
#     calculate_metrics("SVM", y_test, y_pred)


# def calculate_metrics(method, label_test, label_pred):
#     print("Accuracy score for ", method,
#           skmetrics.accuracy_score(label_test, label_pred))
#     print("Precision_score for ", method, skmetrics.precision_score(
#         label_test, label_pred, average='micro'))
#     print("f1 score for ", method, skmetrics.f1_score(
#         label_test, label_pred, average='micro'))
#     print("Recall score for ", method, skmetrics.recall_score(
#         label_test, label_pred, average='micro'))
...
mini_kmeans_model = mini_kmeans(
    ipu.N_CLASSES * ipu.CLUSTER_FACTOR, np.array(all_train_dis))

del all_train_dis


print('Collecting visual words for train .....')
train_images_visual_words = [mini_kmeans_model.predict(
    visual_words) for visual_words in train_img_disc]
print('Visual words for train data collected. length is %i' %
      len(train_images_visual_words))

print('Collecting visual words for test .....')
test_images_visual_words = [mini_kmeans_model.predict(
    visual_words) for visual_words in test_img_disc]
print('Visual words for test data collected. length is %i' %
      len(test_images_visual_words))


print('Calculating Histograms for train...')
bovw_train_histograms = np.array([np.bincount(
    visual_words, minlength=ipu.N_CLASSES * ipu.CLUSTER_FACTOR) for visual_words in train_images_visual_words])
print('Train histograms are collected. Length : %i ' %
      len(bovw_train_histograms))

print('Calculating Histograms for test...')
bovw_test_histograms = np.array([np.bincount(
    visual_words, minlength=ipu.N_CLASSES * ipu.CLUSTER_FACTOR) for visual_words in test_images_visual_words])
print('Test histograms are collected. Length : %i ' %
      len(bovw_test_histograms))
